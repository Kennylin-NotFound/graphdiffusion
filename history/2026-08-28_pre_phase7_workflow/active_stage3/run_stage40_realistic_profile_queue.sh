...
DATASET_REL="${DATASET_REL:-artifacts/datasets/phase6e-e-realistic-profile}"
CONFIG_REL="${CONFIG_REL:-configs/dataset_phase6e_e_realistic_profile.yaml}"
OUT_REL="${OUT_REL:-artifacts/phase6e-e-realistic-profile-evaluation-10seed}"
PARTITIONS="${PARTITIONS:-profile_id,profile_branched,profile_high_sharing}"
...
OUT="$ROOT/$OUT_REL"
SCRIPT="$ROOT/scripts/80_run_realistic_profile_simulation.py"
...
if [[ "$PREPARE_DATA" == "1" ]]; then
  (
    cd "$ROOT"
    "$PY" "$ROOT/scripts/79_prepare_realistic_profile_simulation.py" \
      --config "$CONFIG_REL" \
      --output-root "$DATASET_REL"
...
for index in "${!SEED_SHARDS[@]}"; do
  ...
  log="$OUT/logs/realistic_profile_gpu${index}.log"
  (
    cd "$ROOT"
