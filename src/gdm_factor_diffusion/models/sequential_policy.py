"""Typed factor-graph policy for autoregressive service placement."""
...
class _SequentialNodeEncoder(nn.Module):
    def __init__(
        self,
        feature_dim: int,
        type_count: int | None,
...
class TypedFactorSequentialPolicy(nn.Module):
    """Predict the next service placement from a partial deployment prefix."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.config = config or SequentialPolicyConfig()
        ...
        self.node_encoder = nn.ModuleDict(
            {
                node_type: _SequentialNodeEncoder(
        ...
        self.decision_context = nn.Linear(2, hidden_dim)
    ...
    def _normalize_target_service(
        ...
        if value.shape != (batch.batch_size,):
            raise ValueError("target_service must be scalar or have shape [B].")
        if (value < 0).any() or (value >= batch.candidate_mask.shape[1]).any():
            raise ValueError("target_service contains an out-of-range service.")
        row = torch.arange(batch.batch_size, device=value.device)
        if not batch.service_mask[row, value].all():
            raise ValueError("target_service must refer to active services.")
        return value

    def _normalize_step_fraction(
        self,
        ...
        if value.ndim == 0:
            value = value.expand(batch.batch_size)
        if value.shape != (batch.batch_size,):
            raise ValueError("step_fraction must be scalar or have shape [B].")
    ...
    def forward(
        ...
        """Return compatible `[B, M, D]` logits for the next placement step."""

        validate_partial_state(state, batch.candidate_mask, batch.service_mask)
        target = self._normalize_target_service(target_service, batch)
        progress = self._normalize_step_fraction(step_fraction, batch)
        graph = batch.graph
        hidden = {
        ...
        decision_dense = torch.zeros(
            (*batch.service_mask.shape, 2),
            dtype=batch.processing_latency.dtype,
            device=batch.candidate_mask.device,
        )
        decision_dense[..., 1] = progress[:, None]
        row = torch.arange(batch.batch_size, device=target.device)
        decision_dense[row, target, 0] = 1.0
        decision_dense = decision_dense * batch.service_mask.unsqueeze(-1).to(
            dtype=decision_dense.dtype
        )
        service_decision_context = torch.zeros(
            (hidden["service"].shape[0], 2),
            dtype=decision_dense.dtype,
            device=decision_dense.device,
        )
        service_decision_context[batch.service_node_index[batch.service_mask]] = (
            decision_dense[batch.service_mask]
        )

        hidden["service"] = (
            hidden["service"]
            + state_context
            + self.decision_context(service_decision_context)
        )
