"""Cross-scale evaluation for the absorbing-MASK solver.

This script reuses the frozen Stage 3.9 ten-seed checkpoints and evaluates
them on a scale-partitioned frozen dataset. It is intentionally separate from
the sealed-ID forward-budget evidence so that scalability/generalization
results do not overwrite the core manuscript evidence.
"""
...
def _limited_indices(dataset: LabeledDeploymentDataset, max_per_partition: int | None) -> list[int]:
    selected: list[int] = []
...
def _load_training_entry(root: Path, training: Mapping[str, Any], seed: int, kind: str) -> Path:
    ...
    for training_seed in selected_seeds:
        ...
        for index in indices:
                ...
                sampled = _sample_proposals(
                    ...
                    generator=torch.Generator(device=device).manual_seed(
                        derive_seed(
                            ...
                            f"cross-scale:{method_id}:{training_seed}:{item.instance.instance_id}",
                        )
                    ),
                )
            ...
            for method_id in baseline_methods:
                ...
                elif method_id == "greedy_local":
                    result = solve_greedy_local(item.instance)
                elif method_id == "latency_aware_heuristic":
                    result = solve_latency_aware_heuristic(item.instance)
                elif method_id == "local_search":
                    result = solve_local_search(item.instance)
                else:
                    raise ValueError(f"Unsupported deterministic baseline: {method_id}")
                method_results[method_id] = {
                    "full": _result_payload(result, pool_best)
                }
...
def _write_report(path: Path, evidence: Mapping[str, Any]) -> None:
    ...
    lines = [
        "# Phase 6E-E Cross-Scale Evaluation",
        "",
        "This report evaluates frozen Stage 3.9 checkpoints on scale-partitioned",
        "instances. It is a generalization/scalability diagnostic rather than the",
        "sealed-ID main result.",
        "",
        "## Dataset",
        "",
    ...
    lines.extend([
        "",
        "## Main Paired Tests By Partition",
        "",
...
def finalize(
    ...
    *,
    ...
    for partition in partitions:
        ...
        if MAIN_DIRECT in methods and MAIN_MASKED in methods:
            paired_by_partition[partition] = {
                "raw": _paired(rows, direct=MAIN_DIRECT, masked=MAIN_MASKED, mode="raw_only", stage="raw"),
                "pre": _paired(rows, direct=MAIN_DIRECT, masked=MAIN_MASKED, mode="full", stage="pre"),
                "final": _paired(rows, direct=MAIN_DIRECT, masked=MAIN_MASKED, mode="full", stage="final"),
            }
        else:
            paired_by_partition[partition] = {}
    ...
    evidence = {
        ...
        "claim_boundary": (
            "This is a cross-scale diagnostic. Use it for scalability and "
            "generalization discussion only after checking partition-level trends."
        ),
    }
    evidence_path = output_root / "cross_scale_evidence.json"
    report_path = output_root / "cross_scale_report.md"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-root",
        default="artifacts/phase6e-e-cross-scale-evaluation",
    )
    parser.add_argument(
        "--partitions",
        default=",".join(DEFAULT_PARTITIONS),
        help="Comma-separated scale partitions.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "random; lean keeps only greedy-local plus Direct/Masked variants."
        ),
    )
